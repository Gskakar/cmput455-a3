...
class CommandInterface:
    ...
    def _cell_at(self, x, y):
        """Return (offboard:bool, value in {0,1,2 or None if offboard})."""
        if x < 0 or y < 0 or x >= self.width or y >= self.height:
            return True, None
        return False, self.board[y][x]

    def _char_matches(self, ch, off, val, me):
        """
        Pattern symbols:
          P: current player's piece
          O: opponent's piece
          _: empty
          *: anything on-board (P or O or _)
          X: off-board only
        """
        if ch == 'X':
            return off
            ...
            with open(path, "r", encoding="utf-8") as f:
                for raw in f:
                    ...
                    try:
                        ...
                        return False
                    pat = pat.strip()
                    self.patterns.append((pat, float(w)))
            return True
        except Exception:
            return False
